    ...
    <SettingsGroup :title="t('settings.mcp.servers')">
      ...
      <ul v-else class="mcp-manager__list">
        <li v-for="server in servers" :key="server.name" class="mcp-manager__server">
          ...
          <div class="mcp-manager__actions"><button v-if="!server.disabled" type="button" :disabled="saving" @click="reconnect(server.name)">{{ t('settings.mcp.refresh') }}</button><button v-if="server.tools.length || server.resources.length || server.prompts.length" type="button" :disabled="saving" @click="expanded = expanded === server.name ? null : server.name">{{ t(expanded === server.name ? 'settings.mcp.hideDetails' : 'settings.mcp.showDetails') }}</button><button type="button" :disabled="saving" :class="{ 'is-danger': !server.disabled }" @click="setEnabled(server.name, server.disabled)">{{ t(server.disabled ? 'settings.mcp.restore' : 'settings.mcp.disable') }}</button></div>
